...
abc = 0
while abc == 0:
    print("اختار رقم المجال اللي تبغاه\n")
    ...
    if d == "1":
        ...
        while h <= b1 :
            ...
            spc01 = " " * (2 - len(str(h+1)))
            spc02 = " " * (2 - len(str(h+2)))
            spch = " " * (10 - len(rav[h]) - len(spc00) - len(str(h)))
            spch1 = " " * (20 - len(rav[h]) - len(spc00) - len(str(h)) - len(spch) - len(str(h+1)) - len(spc01) - len(rav[h+1]))
            if h+3 < b1 :
            	print ( h ,spc00,": " , rav[h] ,spch, h+1 ,spc01,": " , rav[h+1] ,spch1, h+2 ,spc02,": " , rav[h+2])
            elif h+2 < b1 :
            	print ( h ,spc00,": " , rav[h] ,spch, h+1 ,spc01,": " , rav[h+1] ,spch1, h+2 ,spc02,": " , rav[h+2])
            elif h+1 <= b1 :
            	print ( h ,spc00,": " , rav[h] ,spch, h+1 ,spc01,": " , rav[h+1] )
            h += 3

        print("اكتب رقم المعطى اللي عندك :\n")
        while a < a1:
            plo = input(": ")
            if plo != "" :
                try :
                	...
                	print("اكتب رقم من 0 الى 22 ولا تخبّص\n\n")
                	continue
                if plo>=0 and plo<a1 :
                	print(rav[int(plo)], " = ")
                	var[int(plo)] = input()
                else :
                	print("يا بطاطس اكتب رقم من 0 الى 22 ولا تخبص\n\n")
            elif plo == "" :
                a = a1
            a += 1
    ...
    # الين هنا يعني.......................
        ...
        print("اكتب بس رقم 1 او رقم 2 (واتأكد انها بالانجليزي)\n\n")
        continue
    ...
    while b != a1:
        if var[b] != "" :
            var[b] = format(float(var[b]) , '.4f')
            spc10 = " " * (2 - len(rav[b]))
            spc20 = " " * (16 - len(spc10) - len(rav[b]) - len(var[b]))
            print(rav[b],spc10,"= ", var[b],spc20, msu[b])
        b += 1
